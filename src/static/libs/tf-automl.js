...
;(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    : factory(
        ((global.tf = global.tf || {}),
        (global.tf.automl = global.tf.automl || {})),
        global.tf,
        global.tf
      )
})(this, function (exports, tfjsCore, tfjsConverter) {
  'use strict'
  ...
  function imageToTensor(img) {
    return img instanceof tfjsCore.Tensor
      ? img
      : tfjsCore.browser.fromPixels(img)
  }
  ...
  class ImageClassificationModel {
    ...
    async classify(input, options) {
      ...
      scores.dispose()
      const result = Array.from(probabilities).map((prob, i) => ({
        label: this.dictionary[i],
        prob,
      }))
      return result
    }
  ...
  async function loadImageClassification(modelUrl) {
    const [model, dict] = await Promise.all([
      tfjsConverter.loadGraphModel(modelUrl),
  ...
  function centerCropAndResize(img) {
    return tfjsCore.tidy(() => {
      ...
      return tfjsCore.image.cropAndResize(
        ...
        tfjsCore.expandDims(tfjsCore.cast(img, 'float32')),
        boxes,
        boxIndices,
        IMG_SIZE
      )
  ...
  class ObjectDetectionModel {
    ...
    async detect(input, options) {
      ...
      const [scoresTensor, boxesTensor] = await this.graphModel.executeAsync(
        feedDict,
        OUTPUT_NODE_NAMES
      )
      ...
      const selectedBoxesTensor = await tfjsCore.image.nonMaxSuppressionAsync(
        boxesTensor,
        boxScores,
        options.topk,
        options.iou,
